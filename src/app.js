...
function App() {
  ...
  useEffect(() => {
    fetchJSON("./data/index.json").then((data) => {
      setIndex(data);
      const premier = data.groupes?.[0]?.modules?.[0]?.cours?.[0];
      if (premier) setCoursSelectionne(premier);
    });
  }, []);
    ...
    h(
      ...
      index.groupes.map((groupe) =>
        h(
          ...
          h("div", { className: "groupe-nom" }, groupe.nom),
          groupe.modules.map((module) =>
            h(
              "div",
              { key: module.id },
              h("div", { className: "module-nom" }, module.nom),
