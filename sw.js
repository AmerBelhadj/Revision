const CACHE_NAME = "revision-pwa-v9";
  ...
  event.waitUntil(
    (async () => {
      ...
      try {
          ...
          for (const module of groupe.modules || []) {
            for (const sousmodule of module.sousmodules || []) {
              for (const cours of sousmodule.cours || []) {
                if (cours.disponible === false) continue;
        ...
        // On met en cache fichier par fichier plutôt qu'avec un seul
        // cache.addAll() : si un seul fichier manque ou échoue,
        // addAll() aurait fait échouer l'installation entière du
        // service worker (donc tout le mode hors-ligne).
        await Promise.all(
